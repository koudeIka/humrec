...
try {
	...
	switch($_POST['action'])
	{
		...
		case 'load_new':
			...
			<fieldset class='new_date'>
				<legend>Date</legend>
				<input type='text' value=\"".substr(date::enToFr($new->new_date), 0, 10)."\" class='ui-state-default' />
			</fieldset>
			<fieldset class='new_content_fr'>
				<legend>Contenu FRENCH</legend>
				<textarea>".$new->new_content_fr."</textarea>
			</fieldset>
			<fieldset class='new_content_en'>
				<legend>Contenu ENGLISH</legend>
				<textarea>".$new->new_content_en."</textarea>
			</fieldset>";
			echo "<fieldset class='new_pictures'>
					<legend>Images</legend>";
					if (count($new->pho_ids)>0)
					{
					    echo "<ul id='photo_list'>";
					    foreach($new->pho_ids as $pho_id)
					    {
			...
			echo "		<div class='menu_img' style='display: none;'>
		...
		case 'save_new_date':
			$new = new news($_POST['new_id']);
			$new->new_date = date::frtoEn($_POST['new_date']);
			$new->save();
			break;

		case 'save_new_content':
			$new = new news($_POST['new_id']);
			if(!empty($_POST['new_content_fr']))
			{
				$new_content_fr = str_replace("%5C%22%5C%22", "", $_POST['new_content_fr']);
				$new_content_fr = str_replace('\\"', "'", $new_content_fr);
				$new_content_fr = str_replace("\\'", "'", $new_content_fr);
				//$new_content_fr = str_replace("\\\"", "", $new_content_fr);
				//$new_content_fr = str_replace("\\", "", $new_content_fr);
				//$new_content_fr = str_replace("/>", ">", $new_content_fr);
				$new->new_content_fr = $new_content_fr;
			}
			if(!empty($_POST['new_content_en']))
			{
				$new_content_en = str_replace("%5C%22%5C%22", "", $_POST['new_content_en']);
				$new_content_en = str_replace('\\"', "'", $new_content_en);
				$new_content_en = str_replace("\\'", "'", $new_content_en);
				//$new_content_en = str_replace("\\\"", "", $new_content_en);
				//$new_content_en = str_replace("\\", "", $new_content_en);
				//$new_content_en = str_replace("/>", ">", $new_content_en);
				$new->new_content_en = $new_content_en;
			}
			$new->save();
			break;
...
/* ############## PHOTOS ####################################### */
		...
		case 'add_pho':
					...
					$newpho_id = $new->addPhoto($_POST['pho_id']);
					$new_has_pho = new new_has_pho($newpho_id);
					$photo = new photos($new_has_pho->pho_id);
					//$relpho_legend = "relpho_legend_".$_POST['lan_code'];

					echo "<li data-pho_id='".$photo->pho_id."' data-sortable='newpho_id_".$new_has_pho->newpho_id."'>
							<img src='../".$INI["photos_path"]["200x200"].$photo->pho_name."' alt='".$photo->pho_name."' class='new_pho' />
							<span class='delete'>&nbsp;</span>
						</li>";
					break;
...
###################################################################*/
	...
	case "show_biblio":
					...
					{
						$photos = photos::getAll($_POST['new_id']);
						$nbpage = ceil(count($photos)/$nb_tofs);
						$start_p1 = 0;
						$start_p2 = $nb_tofs;
						$pho_p2 = photos::getAll($_POST['new_id'], $start_p1, $nb_tofs);
						$pho_p3 = photos::getAll($_POST['new_id'], $start_p2, $nb_tofs);
					...
					else
					{
						if(!empty($_POST['page_num']))
						{
							...
							$photos = photos::getAll($_POST['new_id'],$start,$nb_tofs,$_POST['search']);
							...
							{
								$photos = photos::getAll($_POST['new_id'], null, null, $_POST['search']);
								$nbpage = ceil(count($photos)/$nb_tofs);
